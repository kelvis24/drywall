...
class SimplifiedDualCapture:
    ...
    def configure_camera(self, cam, camera_index):
        ...
        try:
            ...
            if camera_index == 1:  # Camera 2
                print(f"\n🔧 Configuring Camera 2 (Serial: {serial_number})")
                
                # Set full speed settings for Camera 2
                roi_width = 720  # Keep width
                roi_height = 150  # Keep height
                roi_offset_x = 0
                roi_offset_y = 246  # Match Camera 1's offset
                initial_fps = 350.0  # Start at full speed
            ...
            # Set buffer count mode to manual with larger buffers for both cameras
            ...
            if PySpin.IsAvailable(buffer_count) and PySpin.IsWritable(buffer_count):
                buffer_count.SetValue(64)  # Both cameras get 64 buffers
                print(f"✅ Camera {camera_index+1}: Buffer count set to 64")
            ...
            if PySpin.IsAvailable(packet_size) and PySpin.IsWritable(packet_size):
                packet_size.SetValue(packet_size.GetMax())  # Maximum packet size for both cameras
                print(f"✅ Camera {camera_index+1}: Set to maximum packet size")

            # Remove packet delay for both cameras for maximum speed
            ...
            if PySpin.IsAvailable(packet_delay) and PySpin.IsWritable(packet_delay):
                packet_delay.SetValue(0)  # No delay between packets
                print(f"✅ Camera {camera_index+1}: Packet delay set to 0")
    ...
    def capture_thread(self, camera_info):
        ...
        RESET_DELAY = 2.0  # Seconds to wait after reset
        CAPTURE_TIMEOUT = 100  # Milliseconds to wait for frame
    ...
    def save_synchronized_frames(self, frame_dict, sync_key):
        """Save synchronized frames from all cameras as separate files for YOLO training"""
        try:
            ...
            for camera_id in sorted_cameras:
                ...
                # Save individual frame with minimal annotations
                filename = f"cam{camera_id}_frame{frame_count:06d}_{timestamp_str}.jpg"
                filepath = os.path.join(self.output_dir, filename)
                
                # Use thread pool for saving to not block capture
                self.save_executor.submit(self._save_sync_image, filepath, frame_bgr, sync_key, camera_id)
            ...
            print(f"🔄 SYNC #{self.stats['synchronized_frames_saved']}: Saved {len(sorted_cameras)} frames | {fps_str}")
    ...
    def _save_sync_image(self, filepath, image, sync_key, camera_id=None):
        """Save synchronized frame (runs in thread pool)"""
        try:
            # Save the image without any annotations for YOLO training
            cv2.imwrite(filepath, image, [cv2.IMWRITE_JPEG_QUALITY, 90])
            
            # Skip ball detection since we're collecting training data
            if camera_id:
                print(f"💾 Saved Camera {camera_id} frame: {os.path.basename(filepath)}")
